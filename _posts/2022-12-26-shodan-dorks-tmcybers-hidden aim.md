---
title: Shodan Dorks [tmcybers Hidden Aim]
layout: post
post-image: /assets/images/bannergit.png
description: My Shodan Dorks hidden aim [Cheatsheet]
tags:
  - CyberSecurity
  - Hacking
  - Shodan
...
#### Expired certificates `ssl.cert.expired:true`  `ssl.cert.subject.cn:example.com`

## Device Type
`device:firewall` 
`device:router` 
`device:wap` `device:webcam` `device:media` `device:"broadband router"` `device:pbx` `device:printer` `device:switch` `device:storage` `device:specialized` `device:phone` `device:"voip"` `device:"voip phone"` `device:"voip adaptor"` `device:"load balancer"` `device:"print server"` `device:terminal` `device:remote` `device:telecom` `device:power` `device:proxy` `device:pda` `device:bridge`
...
`product:"cobalt strike team server"` `product:"Cobalt Strike Beacon"` `ssl.cert.serial:146473198` - default certificate serial number `ssl.jarm:07d14d16d21d21d07c42d41d00041d24a458a375eef0c576d23a7bab9a9fb1`
...
E* Example: Logitech Media Servers
...
# Dork 4 Pivotal and Imperative One!

`http.title:"Whoops! There was an error." org:"Amazon.com"`

Smtp Crendials And AWS Secret Keys.

This leads us to find sensitive data on Exposure Vulnerability.



---
>>> this is all for now, more to come..stay tuned!@
