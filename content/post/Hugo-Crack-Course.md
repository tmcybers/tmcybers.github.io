...
Fui `Jekill Blogger` desde siempre, un dia me puse a jugar/testear con hugo y probandolo en consola y en localhost, pues me ha gustado y mucho, de hugo puedo decir que mas me gusta es el minimalismo de las temas y su comunidad, y tmb su facilidad y el modus-operandi es `cool`, entonces dije vamos a probarlo, decidi no migrar mi blog, y partir desde `0` , con hugo.
...
Queria levantar el blog, alojarlo en github pages obviamente, ademas de ello conectarlo/alojarlo en cloudflare(gratis) (seguridad+dominio+spam+reglas+firewall) mas el dominio p[ersonalizado (para ello compre el dominio de cloudflare directamente, mas que recomendado).
...
* Cloudflare account (free, lo mismo con el free cloudflare mola)
...
Necesitamos hugo y git instalados: (en mi caso en `arch` uso `yay` mi preferido, si estas en debian te sonara `sudo apt`, y en mac `brew install hugo`, y para windows usa WSL y choco)
...
cd /myblog
```
Listo, en tan solo unos seg. hugo creara un blog/site y veras una multitud de folders, carpetas y archivos sin con-senso, pero chill, ya entenderas.

No hagas nada, dejalos ahy, y vamos a darle vida y aplicare un tema que te guste ;)

*Otro aspecto importante, usa una ruta de tu local o en la nube, yo uso Mega encryptado y lo alojo ahy, asi nunca perdere mi blog, aunque le tengo un back up hecho en otro sitio tmb :) aprende a hacer back-ups, copias, siempre, es una buena practica.*
...
```
git init  #inicia un .git vacio, un repo en tu local.
...
Estamos instalando/aplicando el tema PaperMod a nuestro blog como submodulo de en github.
...
Dentro de config.toml, config.yml, .yaml tienes que anadir:
```
...
Incluso mas facil podemos hacer un: `echo "theme = 'PaperMod'" >> config.toml` y listo. #debes estar en el dir /themes
...
Te recomiendo que le eches un vistazo, y que aprendas como publicar articulos (en hugo se llaman /post o /posts), editarlos, crear mas paginas de las que viene por defecto, y un monton de cosas, hugo las tiene bien documentadas y escritas. 
...
tanto ahora como en un futuro nunca hagas `hugo` de sitio,  hugo creara tu sitio estatico, hara el build de tu blog/sitio en un dir llamado /public, esa carpeta/directorio es tu blog publico sin deployment, que la tienes que alojar en un servidor/servicio como netlify, heroku, back4app, etc. 

### Este paso es importantisimo a dia de hoy, porque github pages ya viene incluido y github hara solito el deploy de tu blog hugo, lo vamos a configurar en el repo por defecto, y listo.


# Breve despliegue y profundismo de un blog en hugo:

`cd ~/myblog  #no olvides myblog es tu directorio /root`

`ls -l #listar carpetas y directorios en listado`
...
git status #buena practica, mira bien el log antes de push--ear y add .
...
git branch -M main #creamos la rama main, trabajaremos en main con git push y deploy;s, no en rama master casi nunca, las buenas practicas ante/todo ;) somos Dev- o no ;)
...
#### Vamos a github a crear el Repo, ve...;): presta atencion aqui , tu repositorio se dede llamar nombre de usuario en github, acorde a ello, mas github.io: `tmcyb3r.github.io`

*Evita que tu blog termine en .github.io/Blog ;)*

Importante en cada repo que creas (palabra de dev.) en github crea el repositorio limpio sin nada, ni siquiera un readme.md, o gitignore, o licencias, dejalo limpio, y pushe-a todo desde local, aunque lo queres crear, creal;o en local y push it, eso te puede dar problemas, con cargas del repo, y work's in progress, por lo menos asi lo interpreta git siendo muy listo, que lo es.
...
git add origin remote  #copias y pegas la direccion de tu repo https:// ..............git```
git push -u origin main #listo hemos empujado todo nuestro blog/site a github 

Diccionario:
#origin(el origen,e el repo, donde? en main) 
#main la rama main (usala siempre)
```

Ve a : la pestana /Actions de tu /repo y veras el deploy en vivo ;) asi como algun error que puede succeder, no descarto, es la vida de /devs.

Actualiza la pagina de tu repo y veras el contenido subido.

Ahora necesitamos un workflow/github gh-pages.yml para la rama gh-pages, a dia de hoy es obsoleto `ojo con esto`, no es necesario, ya que github la tiene como incluida:
(Esto si no tienes pensado en alojar tu blog/sitio en github, si es otro cliente/servicio entonces posbile que necesites otro tipo de setting)
...
* 4.Github Actions de nuevo y elije `Hugo` te aparecera en grande:) pincha en complete y commit de ello en tu repo, y listo, github se encargara del build-ing y deploy de tu blog/site, no tienes que hacer nada mas, eso viene por defecto en github asociado a github-pages tanto en hugo como jekill.

El custom domain, si lo tienes de otro provedor que no sea cloudflare, tienes que asignarlo con DNS check (DNS Check in Progress), esto segun cada cliente, en cloudflare se hace en la pagina de ellos, y no en github, ya veras:0crack.
...
cd ~/myblog #/root blog.
ls #list
git init  #iniciamos por si acaso.
git status #ya sabes, peso pesado;)
git add . #push todo.
git commit -m "baseURL anadido" #comenta tu jobs siempre.
git push #push -it.
...
En este paso tienes la cuenta de cloudflare, si no tienes una create-la, es free, no worry (tuve que usar una vpn para ello, de alguna manera cloudflare no le tenia carino a mi ip :) asi que usa una vpn para ello y listo)
...
No subimos nada,solo conectamos el github/gitlab nuestro con cloudflare y lo alojamos en cloudflare para una mayor seguridad y en contra del spam y muchisimo mas (recomiendo cloudflarea todo paso, aunque aveces odiamos los captchas en javascript de ellos:), solo conectas next, next:

Pon un nombre descriptivo hugo-sample es solo de ejemplo;), usa nombre de usuario, o nombre personalizado que le pusiste a tu dominio:
...
Para ello deberas ir a la pestana: Custom Domain y asignar o comprar uno.
